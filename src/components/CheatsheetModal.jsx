...
export default function CheatsheetModal({ unitTitle, terms, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
      ...
      {/* Modal */}
      <div
        className="relative bg-surface/90 backdrop-blur-xl border border-border/60 rounded-3xl max-w-4xl w-full max-h-[85vh] overflow-hidden animate-scale-in shadow-2xl flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-border/50 bg-surface/50">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center border border-primary/20">
              <BookOpen className="w-5 h-5 text-primary" />
            </div>
            <div>
              <h2 className="text-xl font-bold tracking-tight text-text leading-none">Unit Cheatsheet</h2>
              <p className="text-[13px] text-text-secondary/80 mt-1 font-medium">{unitTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-surface-hover hover:text-danger transition-colors text-text-secondary"
        ...
        <div className="overflow-y-auto flex-1 p-6 custom-scrollbar">
          <div className="rounded-2xl border border-border/50 overflow-hidden bg-bg/30">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border/50 bg-surface/50">
          <p className="text-[13px] text-text-secondary/70 flex items-center gap-2 font-medium">
          ...
          <button onClick={onClose} className="btn-outline text-sm font-semibold px-6 py-2.5 hover:bg-surface-hover border-border hover:border-border-hover hover:text-text">
            Close
          </button>
        </div>
      </div>
